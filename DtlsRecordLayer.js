...
DtlsRecordLayer.prototype.send = function(msg, callback) {
    ...
    for (var m in msg) {
        ...
        if (
            msg[m].type === dtls.MessageType.changeCipherSpec &&
            !msg[m].__sent
        ) {
...
DtlsRecordLayer.prototype.sendInternal = function(buffers, callback) {
    ...
    while (buffers.length > 0) {
        if (buffers[0].length + flight_length > 1000) {
            pending++;
            this.dgram.send(
                Buffer.concat(flight),
                0,
                flight_length,
                this.rinfo.port,
                this.rinfo.address,
                singlePacketCallback
            );
    ...
    pending++;
    this.dgram.send(
        flight.length === 1 ? flight[0] : Buffer.concat(flight),
        0,
        flight_length,
        this.rinfo.port,
        this.rinfo.address,
...
DtlsRecordLayer.prototype.decrypt = function(packet) {
    ...
    var decrypted = Buffer.concat([cipher.update(ciphered), cipher.final()]);
    ...
    var expectedMac = parameters.calculateIncomingMac([
        header,
        packet.fragment
    ]);
...
DtlsRecordLayer.prototype.encrypt = function(packet) {
    ...
    var blockSize = 16;
    var overflow =
        (iv.length + packet.fragment.length + mac.length + 1) % blockSize;

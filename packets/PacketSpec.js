...
PacketSpec.normalize = function(item) {
    ...
    var hasValidType =
        item.type &&
        ((item.type.prototype && item.type.prototype.spec) || types[item.type]);
    var hasReadWrite = item.read && item.write;

    if (!(hasValidType || hasReadWrite)) {
        throw new Error(
            "PacketSpec must have either a valid type or read/write functions, field: " +
                item.name
        );
    }

    if (item.itemType) item.itemType = PacketSpec.normalize(item.itemType);
